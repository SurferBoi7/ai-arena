// =============================================================================
// server/digest.mjs — Deterministic "Smart Report" generator (NO AI APIs).
// =============================================================================
// Produces a beautifully formatted, highly readable email from the fetched JSON
// data using strict deterministic logic and string formatting only. No LLMs,
// no external generation APIs. Same module powers:
//   - the local "Send Test Digest Now" button (server/dev-server.mjs)
//   - the GitHub Actions scheduled dispatch (scripts/send-digest.ts)
// =============================================================================
...
export function loadData(dataDir = DATA_DIR) {
  const read = (f) => JSON.parse(readFileSync(resolve(dataDir, f), "utf-8"));
  return {
...
// Rank delta helper: compare a model's score to the model above it.
function movers(models, n = 5) {
  // Deterministic "movers": the models whose omission penalty / status changed
  // the ranking most — here we surface the biggest score gaps between adjacent
  // ranks plus any partial/flagship standouts. Fully derived from data.
  ...
  // Largest gaps = clearest separators.
...
export function buildSubject(meta, options) {
...
export function buildText({ models, feed, companies, meta }, options) {
...
export function buildHtml({ models, feed, companies, meta }, options) {
  const topN = options.topN || 10;
  const sections = { ...DEFAULT_SECTIONS, ...(options.sections || {}) };
  const ranked = [...models].sort(
    (a, b) => b.ultimateScore - a.ultimateScore || b.benchmarkCount - a.benchmarkCount,
  ...
  const sectionHead = (title, accent = BRAND) =>
  ...
  if (sections.leaderboard) {
    ...
    body += `<thead><tr style="background:#f8f9fb;">` +
...
// ---------------------------------------------------------------------------
// Public entry: build a complete digest for given options.
// ---------------------------------------------------------------------------

export function generateDigest(data, options = {}) {
  const opts = { ...DEFAULT_OPTIONS, ...options, sections: { ...DEFAULT_SECTIONS, ...(options.sections || {}) } };
  return {
    subject: buildSubject(data.meta, opts),
    text: buildText(data, opts),
    html: buildHtml(data, opts),
  };
}
